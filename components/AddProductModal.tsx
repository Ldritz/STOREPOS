...
import type { InventoryItem, FeedsSubtype, NonFeedsSubtype } from '../types.ts';
...
interface AddProductModalProps {
  onClose: () => void;
  onAddProduct: (newProduct: Omit<InventoryItem, 'id'>, supplierPrice: number) => void;
}
...
const AddProductModal: React.FC<AddProductModalProps> = ({ onClose, onAddProduct }) => {
  ...
  const [category, setCategory] = useState<'Feeds' | 'Non-Feeds'>('Feeds');
  const [subtype, setSubtype] = useState<FeedsSubtype | NonFeedsSubtype>('Kilo');
  ...
  useEffect(() => {
    ...
    if (sp > 0 && qty > 0) {
      ...
      const calculatedPrice = unitCost / margin;
      setSellingPrice(Math.round(calculatedPrice));
    } else {
      setSellingPrice(0);
    }
  }, [supplierPrice, quantity, margin]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!name.trim() || !supplierPrice || supplierPriceNum <= 0 || !quantity || quantityNum <= 0 || sellingPrice <= 0) {
      alert('Please fill out all fields correctly.');
      return;
    }

    const unitCost = supplierPriceNum / quantityNum;

    const newProduct: Omit<InventoryItem, 'id'> = {
        name,
        category,
        subtype: subtype as any, // Cast because TS can't narrow it down from state
        stock: quantityNum,
        price: sellingPrice,
        imageUrl,
        unitCost,
        supplierPrice: supplierPriceNum,
        quantityInBatch: quantityNum,
    };

    onAddProduct(newProduct, supplierPriceNum);
  };
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-lg">
        <form onSubmit={handleSubmit}>
          <header className="flex justify-between items-center p-4 border-b border-slate-200 dark:border-slate-700">
            <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">Add New Product</h2>
          ...
          <main className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
            <InputField label="Product Name" id="productName">
              <input type="text" id="productName" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Grower Pellets" required className="w-full px-3 py-2 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary-400 focus:border-transparent outline-none placeholder:text-slate-400 dark:placeholder:text-slate-500" />
            </InputField>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <InputField label="Total Supplier Price (₱)" id="supplierPrice">
                    <input type="number" id="supplierPrice" value={supplierPrice} onChange={e => setSupplierPrice(e.target.value)} placeholder="e.g., 1000" min="0.01" step="0.01" required className="w-full px-3 py-2 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary-400 focus:border-transparent outline-none placeholder:text-slate-400 dark:placeholder:text-slate-500" />
                </InputField>
                <InputField label="Quantity in Batch" id="quantity">
                    <input type="number" id="quantity" value={quantity} onChange={e => setQuantity(e.target.value)} placeholder="e.g., 50" min="1" step="1" required className="w-full px-3 py-2 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary-400 focus:border-transparent outline-none placeholder:text-slate-400 dark:placeholder:text-slate-500" />
                </InputField>
            </div>

             <div className="space-y-2">
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Product Image (Optional)</label>
            ...
            <div className="!mt-6 bg-primary-50 dark:bg-primary-900/20 p-4 rounded-lg text-center">
                <label htmlFor="sellingPrice" className="block mb-1 text-sm font-medium text-primary-800 dark:text-primary-200">Calculated Selling Price (per item)</label>
                <div className="relative mt-1">
                    <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-slate-500 dark:text-slate-400 text-xl">₱</span>
                    <input type="text" id="sellingPrice" value={sellingPrice > 0 ? String(sellingPrice) : '0'} disabled className="w-full pl-8 pr-3 py-2 bg-white dark:bg-slate-700/50 border-0 rounded-lg font-bold text-2xl text-primary-700 dark:text-primary-300 text-center" />
                </div>
                <p className="text-xs text-primary-700 dark:text-primary-300 mt-2">Based on a {((1 - margin) * 100).toFixed(0)}% profit margin.</p>
            </div>
          </main>
